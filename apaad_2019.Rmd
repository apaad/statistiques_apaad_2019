---
title: "Statistiques APAAD - 10/05/2020"
output:
  html_document:
    toc: true
...
knitr::include_graphics("img/logo_apaad.png")
```
...
```{r}
# On omet cette partie pour des raisons d'anonymat.
...
On exclut ici les femmes transférées (transfert néonatal/perpartum/postpartum).
...
hist_tpp <- ggplot(data = don_tpp %>% filter(transfert_post_partum_maternel == 1)) +
...
don_tpp %>%
  filter(transfert_post_partum_maternel == 1) %>%
...
D'après le test LRT, la parité est statistiquement significative au seuil de 1%. Une grande partié diminue la probabilité de transfert post partum maternel.
...
hist_tpp_neo <- ggplot(data = don_tpp_neo %>% filter(transfert_post_partum_neonatal == 1)) +
...
don_tpp_neo %>%
  filter(transfert_post_partum_neonatal == 1) %>%
