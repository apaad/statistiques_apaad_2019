...
don_profil_groupes_transfert %>%
  ...
  mutate(
    Proportion = (100*Nombre/sum(Nombre)) %>% round(2) %>% str_c(' %'),
    `Parité` = if_else(`Parité`, 'Strictement moins de 5', '5+')
    ) %>%
  kable() %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
...
La p-value est approximativement égale à `r chi2_uterus_cicatriciel_hemo$p.value`. On ne rejette pas l'hypothèse d'indépendance $H_0$ au seuil de signification 5%. Le test ne permet pas de conclure qu'il existe une dépendance statistique entre les deux variables étudiées.

# Transfert anténatal

## Lien entre transfert anténatal, âge et parité

On effectue une régression logistique où la variable à expliquer est le transfert anténatal (1 : transfert / 0 : pas de transfert), et les variables explciatives sont l'âge de la mère et la parité.

```{r}
don_transfert_antenatal_age_parite <- don %>%
  select(transfert_antenatal, age_mere, parite) %>%
  drop_na() %>% # Seulement 1 NA pour 1323 individus, on se permet de l'exclure.
...
don_phases %>%
  ...
  summarise(
    `Durée moyenne` = mean(Durée, na.rm = TRUE) %>% round(2),
    `Durée minimale` = min(Durée, na.rm = TRUE) %>% round(2),
    `Durée maximale` = max(Durée, na.rm = TRUE) %>% round(2),
    `Écart-type` = sd(Durée, na.rm = TRUE) %>% round(2),
  ) %>%
  kable() %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
...
don_rupture %>%
  ...
  summarise(
    Nombre = n(),
    `Durée travail - Min (min)` = min(duree_travail_1_latence + duree_travail_2_active, na.rm = TRUE),
    `Durée travail - Max (min)` = max(duree_travail_1_latence + duree_travail_2_active, na.rm = TRUE),
    `Durée travail - Moy. (min)` = round(mean(duree_travail_1_latence + duree_travail_2_active, na.rm = TRUE),2),
    `Durée travail - E.T. (min)` = round(sd(duree_travail_1_latence + duree_travail_2_active, na.rm = TRUE),2)
  ) %>%
  mutate(
    Proportion = (100*Nombre/sum(Nombre)) %>% round(2) %>% str_c(' %')
    ) %>%
...
```{r fig.align="center"}
hist_rupture <- ggplot(data = don_rupture %>% drop_na(rupture_poche)) +
...
On procède à un test du $\chi^2$ d'indépendance entre les deux variables **Couleur du liquide amniotique** et **transfert per-partum OU post-partum**. Dans cette partie, on exclut les individus tels que **Couleur du liquide amniotique** = *Autre ( sanglant, citrin ... )*. L'hypothèse nulle est $H_0$ : les deux variables sont indépendantes.
...
```{r}
don_presentation <- don_travail # %>% drop_na(presentation)
...
```{r}
don_position_accouchement <- don_accouchement # %>% drop_na(position_accouchement)
...
Il faut exclure la *position gynécologique* par manque d'individus. La proportion de périnée intact est la plus élevée pour cette position. Il faut néanmoins nuancer cette observation en prenant en compte le faible nombre d'individus concernés.
...
La p-value est nettement supérieure à 0.05, égale à `r position_perinee_chi2$p.value`. On ne rejette pas l'hypothèse d'indépendance au seuil de 5 %. Les variables de *position d'accouchement* et d'*état du périnée* semblent statistiquement indépendantes.
...
En particulier, on peut noter que pour `r nbre_duree_expulsion_inf_30` femmes (`r round(nbre_duree_expulsion_inf_30 / nrow(don_accouchement), 2)` % ayant effectivement accouché à domicile), la durée d'expulsion a été inférieure ou égale à 30 minutes.
...
La durée d'expulsion paraît beaucoup plus importante en position gynécologique. Le nombre d'individus est toutefois très inférieur aux effectifs des autres groupes de position.
...
don_hands <- don_accouchement %>%
  ...
  mutate(
    ...
    perinee = case_when(
      ...
      str_sub(perinee, 1, 1) %in% c('2', '3', 'E') ~ 'Degré 2 / Degré 3 / Épisiotomie',
      str_sub(perinee, 1, 1) == 'I' ~ 'Intact',
      TRUE ~ perinee
    )
  )
...
don_hands_perinee <- don_hands %>%
  ...
  mutate(
    perinee = case_when(
      str_sub(perinee, 1, 1) == '1' ~ 'Degré 1',
      str_sub(perinee, 1, 1) %in% c('2', '3', 'E') ~ 'Degré 1 / Degré 2 / Épisiotomie',
      str_sub(perinee, 1, 1) == 'I' ~ 'Intact',
      TRUE ~ perinee
    )
  )
...
D'après le test LRT, la significativité statistique de la parité est particulièrement importante pour expliquer le transfert per partum. En effet la p-value associée est extrêmement faible et le coefficient associé (dans la régression) relativement important. L'âge de la mère est lui aussi une variable statistiquement significative. Son coefficient est toutefois proche de 0 dans la régression logistique. Ces conclusions sont à prendre avec réserve étant donnée l'assymétrie des résidus.
...
# Délivrance pour les femmes n'ayant pas fait l'objet d'un transfert post-partum immédiat
...
On effectue une régression logistique où la variable à expliquer est le transfert post-partum néonatal (1 : transfert / 0 : pas de transfert), et les variables explicatives sont les suivantes : Durée d'expulsion / Poids / Parité / Âge. Il faut noter que les données sont très déséquilibrées, avec seulement 16 transfert post-partum maternels pour 918 mères non-transférées. Cette situation a des conséquences négatives sur la fiabilité du modèle construit ci-dessous.
...
## Hospitalisation dans les 8 jours suivant l'accouchement
